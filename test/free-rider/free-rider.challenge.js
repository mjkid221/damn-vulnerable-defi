...
describe("[Challenge] Free Rider", function () {
  ...
  /**
   * The challenge is about identifying the vulnerability within the NFT marketplace contract.
   * The marketplace contract has a logic bug where payment is repaid to the buyer once the NFT has been transferred.
   * This means we can keep purchasing the NFTs as long as we have enough ETH. But we only have 0.1 ETH, and each NFTS are worth 15 ETH.
   * How do we get the NFTs for free? The answer is to use uniswap's flash loan via tge swap function (https://github.com/Uniswap/v2-core/blob/master/contracts/UniswapV2Pair.sol).
   * An integral feature of Uniswap V2 is that all swaps can be used for flash loan, where as long as payment is fulfilled in the transaction, we can run arbitrary logic in the uniswapV2Call callback function.
   * In this case, we can loan enough WETH (15), drain the marketplace, and then repay the loan with the fees.
   */
  it("Execution", async function () {
    ...
    await (
      await freeRiderAttacker.connect(player).attack(NFT_PRICE, {
        value: PLAYER_INITIAL_ETH_BALANCE / BigInt(2),
      })
    ).wait();
  });
